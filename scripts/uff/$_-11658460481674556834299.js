(function(){{
	 ...
	 * @returns {HTMLElement} dom
	 */
        $that.remove = function remove(dom) {
        var index;
        if (this.has(dom)) {
	 ...
	 */
        $that.place = function place(newParentDom) {
	 ...
	 * @returns {HTMLElement} dom
	 */
        $that.move = function move(dom, position) {
        if (this.has(dom)) {
	 ...
	 * @returns {HTMLElement} dom
	 */
        $that.insert = function insert(dom, position) {
        if (!this.has(dom)) {
            _childNodes.splice(position, 0, dom);
	 ...
	 * @returns {HTMLElement} dom
	 */
        $that.getPosition = function getPosition(dom) {
	 ...
	 * @returns {HTMLElement} dom
	 */
        $that.has = function has(childDom) {
	 ...
	 */
        $that.getParent = function getParent() {
        return _parent;
    };
    /**
	 ...
	 * @private
	 */
        $that._setParent = function _setParent(parent) {
        _parent = parent;
        return _parent;
    };
    /**
	 ...
	 * @private
	 */
        $that.getHidePlace = function getHidePlace() {
        return _hidePlace;
    };
    /**
	 ...
	 * @private
	 */
        $that.setHidePlace = function setHidePlace(hidePlace) {
	 ...
	 */
        $that.getLastTransit = function getLastTransit() {
        return _lastTransit;
    };
    /**
	 ...
	 */
        $that.transit = function transit(dom) {
        if (_lastTransit) {
