(function(){{
    var EventDispatcher = require('lavaca/events/EventDispatcher'), Connectivity = require('lavaca/net/Connectivity'), ArrayUtils = require('lavaca/util/ArrayUtils'), Cache = require('lavaca/util/Cache'), Promise = require('lavaca/util/Promise'), clone = require('mout/lang/deepClone'), merge = require('mout/object/merge'), Config = require('lavaca/util/Config');
    var UNDEFINED;
   ...
   */
    var Model = EventDispatcher.extend(function (map) {
        ...
        if (map) {
            this.suppressEvents = this.suppressTracking = true;
            this.apply(map);
            this.suppressEvents = this.suppressTracking = false;
        }
    }, {
        /**
     ...
     */
        get: function (attribute) {
            var attr = this.attributes.get(attribute), flags;
     ...
     */
        ...
        set: function (attribute, value, flag, suppress) {
            return _suppressChecked(this, suppress, function () {
                ...
                var previous = this.attributes.get(attribute), messages = this.suppressValidation ? [] : this.validate(attribute, value);
                if (messages.length) {
                    ...
                    if (previous !== value) {
                        ...
                        if (!this.suppressTracking && !ArrayUtils.contains(this.unsavedAttributes, attribute)) {
     ...
     */
        clear: function (flag) {
            if (flag) {
                var attrs = this.flags[flag], i = -1, attr, item;
                if (attrs) {
                    while (!!(attr = attrs[++i])) {
     ...
     */
        fetch: function (url, options) {
            ...
            return Promise.when(this, Connectivity.ajax(options)).success(this.onFetchSuccess).error(this.onFetchError);
        },
        /**
     ...
     */
        ...
        save: function (callback, thisp) {
            var promise = new Promise(this), attributes = this.toObject(), changedAttributes = {}, i = -1, attribute, result;
     ...
     */
        toObject: function () {
            var obj = this.attributes.toObject(), flags;
            for (var key in obj) {
