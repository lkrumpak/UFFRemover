(function(){{
	 ...
	 */
        $that.setSocket = function setSocket(socket) {
	 ...
	 */
        $that.getSocket = function getSocket() {
        return _socket;
    };
    /**
	 ...
	 */
        $that.on = function on(event, func) {
	 ...
	 * @param {Function} func the function to execute when the event fires
	 */
        $that.once = function once(event, func) {
	 ...
	 */
        $that.emit = function emit(event, data, callback) {
	 ...
	 * @param {Function} callback is the function to be called for ack
	 */
        $that.removeListener = function removeListener(event, data, callback) {
	 ...
	 */
        $that.request = function request(channel, data, func, scope) {
        if (typeof channel == 'string' && typeof data != 'undefined') {
            var reqData = {
	 ...
	 * @returns
	 */
        $that.listen = function listen(channel, data, func, scope) {
        if (typeof channel == 'string' && typeof data != 'undefined' && typeof func == 'function') {
            var reqData = {
                    eventId: Date.now() + Math.floor(Math.random() * 1000000),
                    data: data,
                    keepAlive: true
