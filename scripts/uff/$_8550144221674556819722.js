(function(){{
    ...
    DateUtils.fn = {
        ...
        h: {
            ...
            i: function (input, dateObj, mappedObj) {
                var h = _int(input) - 1, tod = (mappedObj.t || mappedObj.tt || 'A').indexOf('A') === 0 ? 0 : 12;
                dateObj.hour = h + tod;
            },
            o: function (date, utc) {
        ...
        z: {
            ...
            o: function (date, padding) {
                var off = date.getTimezoneOffset(), offH = Math.floor(Math.abs(off / 60));
        ...
        zzz: {
            ...
            o: function (date) {
                var z = date.getTimezoneOffset(), sign = z > 0 ? '-' : '+', m = z % 60, h = Math.abs((z - m) / 60);
                return sign + _pad(h, 2) + ':' + _pad(Math.abs(m), 2);
            }
        }
    };
    function _parseFormat(f) {
        var actors = [], buffer = '', i = -1, bufferChar, c;
        while (!!(c = f.charAt(++i))) {
   ...
   */
    DateUtils.parse = function (s, f) {
        var actors = _parseFormat(f), exp = _actorsToRegex(actors), dateObj = {
                year: 0,
                month: 0,
                date: 1,
                hour: 0,
                ...
                ms: 0,
                offset: 0
            }, mappedObj = {}, i = -1, actor, match, handler;
        if (exp.test(s)) {
            match = exp.exec(s);
            while (!!(actor = actors[++i])) {
        ...
        return new Date(DateUtils.monthsAbbr[dateObj.month] + ' ' + _pad(dateObj.date, 2) + ' ' + _pad(dateObj.year, 4) + ' ' + _pad(dateObj.hour, 2) + ':' + _pad(dateObj.minute, 2) + ':' + _pad(dateObj.second, 2) + (dateObj.ms > 0 ? '.' + _pad(dateObj.ms, 3) : '') + (dateObj.offset >= 0 ? '+' : '-') + _pad(Math.floor(Math.abs(dateObj.offset / 60)), 2) + _pad(Math.abs(dateObj.offset % 60), 2));
    };
    /**
   ...
   */
    DateUtils.stringify = function (d, f, utc) {
        var actors = _parseFormat(f), i = -1, s = [], actor, handler;
        while (!!(actor = actors[++i])) {
            handler = DateUtils.fn[actor];
            if (handler) {
