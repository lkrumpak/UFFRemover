(function(){{
     ...
     */
        _stateMachine = new StateMachine('Pending', {
            ...
            'Pending': [
                ...
                [
                    ...
                    function onFulfill(value) {
                        _value = value;
                        _observable.notify('fulfill', value);    // Then it transits to the fulfilled state
                    },
                    'Fulfilled'
                ],
                ...
                [
                    ...
                    function onReject(reason) {
                        _reason = reason;
                        _observable.notify('reject', reason);    // Then it transits to the rejected state
                    },
                    'Rejected'
                ],
                ...
                [
                    'toReject',
                    function toReject(resolver) {
                        _observable.watch('reject', resolver);
                    }
                ]
            ],
            // When fulfilled,
            'Fulfilled': [// We directly call the resolver with the value
                [
                    'toFulfill',
                    function toFulfill(resolver) {
                        resolver(_value);
            ...
            // When rejected
            'Rejected': [// We directly call the resolver with the reason
                [
                    'toReject',
                    function toReject(resolver) {
                        resolver(_reason);
     ...
     * @returns the promise
     */
        $that.fulfill = function fulfill(value) {
        setTimeout(function () {
     ...
     */
        $that.reject = function reject(reason) {
        setTimeout(function () {
     ...
     */
        ...
        if (!(arguments[1] instanceof Function) && !(arguments[2] instanceof Function)) {
     ...
     */
        $that.cast = function cast(thenable) {
        if (thenable instanceof PromiseConstructor || typeof thenable == 'object' || typeof thenable == 'function') {
            thenable.then(this.fulfill.bind(this), this.reject.bind(this));
            return true;
        } else {
            return false;
        }
     ...
     */
        $that.makeResolver = function makeResolver(promise, func, scope) {
     ...
     * @private
     */
        $that.getReason = function getReason() {
        return _reason;
    };
    /**
     * Returns the reason
     ...
     * @private
     */
        $that.getValue = function getValue() {
        return _value;
    };
    /**
     ...
     * @returns {Observable}
     */
        $that.getObservable = function getObservable() {
        return _observable;
    };
    /**
     ...
     */
        $that.getStateMachine = function getStateMachine() {
     ...
     * @returns {Object}
     */
        $that.getStates = function getStates() {
        return _states;
    };
}})();