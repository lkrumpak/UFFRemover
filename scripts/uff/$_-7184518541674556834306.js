(function(){{
     ...
     * @returns handle
     */
        $that.watch = function watch(topic, callback, scope) {
        if (typeof callback == 'function') {
            var observers = _topics[topic] = _topics[topic] || [], observer = [
                    callback,
                    scope
                ];
     ...
     * @returns handle
     */
        $that.once = function once(topic, callback, scope) {
     ...
     */
        $that.notify = function notify(topic) {
        var observers = _topics[topic], args = toArray(arguments).slice(1);
        if (observers) {
     ...
     */
        $that.hasObserver = function hasObserver(handle) {
     ...
     */
        $that.hasTopic = function hasTopic(topic) {
     ...
     */
        $that.unwatchAll = function unwatchAll(topic) {
        if (_topics[topic]) {
            delete _topics[topic];
        } else {
            _topics = {};
