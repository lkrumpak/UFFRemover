...
'''
def get_true_dead_functions(subject, uffs):
  DEAD_FUNCTIONS_PATH = EXAMPLES_GROUNDTRUTH_PATH + subject + '/_dead_functions.json'

  with open(DEAD_FUNCTIONS_PATH, 'r') as f1:
    dead_data = json.load(f1)

  print('There are %d UFFs for this subject' % len(uffs))

  true_dead_functions = 0
  for obj in dead_data:
    ...
    for uff in uffs:
      ...
      if match_counter >= nonmatch_counter:
        true_dead_functions += 1
        uffs.remove(uff)
        break;
  
  print('true_dead_functons: %d ' % true_dead_functions)
  false_dead_functions = len(dead_data) - true_dead_functions
  return true_dead_functions, false_dead_functions
...
def get_vazquez_stats():
  ...
  with open('_statistics_.csv', 'a', newline='') as f:
    ...
    CSV_HEADER = [['Subject','AllFunctions','ClaimedAllFunctions','DeadFunctions','ClaimedDeadFunctions','CreatedUFFS', 'TrueDeadFunctions', 'FalseDeadFunctions']]
    writer.writerows(CSV_HEADER)

    #loop through all of the available subjects
    for subject in sorted(SUBJECTS):
      if subject in ['dojo', 'somajs', 'troopjs_require']:
        continue
      # if subject not in ['sapui5']:
      #   continue
      ...
      true_dead_functions, false_dead_functions = get_true_dead_functions(subject, uffs)

      # Retrieve the output stats from vazquez tool
      vq_all, vq_alive, vq_dead = get_subject_stats(subject)

      # Write data to csv file
      # 'Subject','AllFunctions','ClaimedAllFunctions','DeadFunctions','ClaimedDeadFunctions','CreatedUFFS', 'TrueDeadFunctions'
      data = [[subject,gt_all_func,vq_all,gt_dead_func,vq_dead,created_uffs,true_dead_functions,false_dead_functions]]
